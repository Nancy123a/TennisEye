...
class MiniCourt():
    ...
    def get_mini_court_coordinates(self,
                                   object_position, ## foot position of player
                                   closest_key_point, ## closest keypoint to the player
                                   closest_key_point_index, ## closet keypoint index to the player
                                   player_height_in_pixels, ## max height of players in pixels
                                   player_height_in_meters ## the original heights of players in meters
                                  ):
        ## calculate the x and y position between the foot position and the closest keypoint to the player in pixels (in the original court)
        distance_from_keypoint_x_pixels, distance_from_keypoint_y_pixels = measure_xy_distance(object_position, closest_key_point)

        # Conver pixel distance to meters (distance between foot of player and closest keypoint in meters)
        ## x coordinate
        ...
        # Convert to mini court coordinates - x and y position of distance between player and closest keypoint in the minicourt
        mini_court_x_distance_pixels = self.convert_meters_to_pixels(distance_from_keypoint_x_meters)
        mini_court_y_distance_pixels = self.convert_meters_to_pixels(distance_from_keypoint_y_meters)
         ## closest keypoint x and y coordinate
        closest_mini_coourt_keypoint = ( self.drawing_key_points[closest_key_point_index*2],
                                        self.drawing_key_points[closest_key_point_index*2+1]
                                        )
        ## get the player position (x and y coordinates) in the mini court = closest keypoint x ,y + distance between player and closest keypoint x,y
        ...
        ## rteurn the x and y coordinate sof player position in minicourt
    ...
    def convert_bounding_boxes_to_mini_court_coordinates(self,player_boxes, ball_boxes, original_court_key_points ):
        ...
        output_player_boxes= []
        output_ball_boxes= []
        closest_keypoints_draw=[]

        for frame_num, player_bbox in enumerate(player_boxes):
            ball_box = ball_boxes[frame_num][1]
            ball_position = get_center_of_bbox(ball_box)
            ## x is either 1 or 2 for players id
            ## closest distance between the players center and ball position
            ## it return the index of player that is closest to the ball
            ...
            ## loop through players bounding box
            for player_id, bbox in player_bbox.items():
                ## distnace between player and closest keypoint
                foot_position = get_foot_position(bbox)

                # Get The closest keypoint in pixels between the court keypoints and foot of player
                ## 0, 2, 12, 13 are the keypoints of the court where 0 and 12 for player2 and 2, 13 to player 1

                ## get cölosest keypoint index to foot of player 
                closest_key_point_index = get_closest_keypoint_index(foot_position,original_court_key_points, [0,2,12,13])
                closest_key_point = (original_court_key_points[closest_key_point_index*2],  ## x position
                                     original_court_key_points[closest_key_point_index*2+1]) ## y position

                # Get Player height in pixels
                ## specify a range to of frames and takes the maximum height

                ## range of frames
                frame_index_min = max(0, frame_num-20)
                frame_index_max = min(len(player_boxes), frame_num+50)

                ## array of player heights in range of frames
                bboxes_heights_in_pixels = [get_height_of_bbox(player_boxes[i][player_id]) for i in range (frame_index_min,frame_index_max)]
                ## the maximum height of player
                max_player_height_in_pixels = max(bboxes_heights_in_pixels)

                ## calculate the position of player in pixels to plot in the mini court
                ...
                ## distance betwee player and ball
                if closest_player_id_to_ball == player_id:
                    ...
                    closest_keypoints_draw.append(closest_key_point)
                    
            output_player_boxes.append(output_player_bboxes_dict)

        return output_player_boxes , output_ball_boxes,closest_keypoints_draw
